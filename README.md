# FS-Graphics
FiveM Graphics Made By (⸸♱♥IDK_FORCE♥♱⸸#7611) (FiveM Graphics)

- :gear:・Join My Discord And Open Ticket To Ask Support or Give Me Suggestions Or Report A Bug

```My Discord```
- [Discord](https://discord.gg/6kJ5ubDEWE)

```
  🎅🎃🍁September/October/November/December/January Updates🍁🎃🎅
```
  - Better Water
  - Better Rain
  - Better Colors
  - Better Reflections
  - Better Clouds
  - Better Nights
  - Better Sky
  - Better Fire
  - Better StreetLights
  - FPS Increase

```
   Things To (Fix/Add)
```
- Random Bloom On Police Vehicles (Fix)
- Raindrops on Vehicles (Fix)
- Dirty Water (Fix)
- Better Fog (Add)
- Rain Puddles (Add)
- Better Roads/Dirt/Grass (Add)
- Better Vegetation (Add)
- Better Texture Mix (Add)
- Better Shadows (Add)
- Retexture More Then 35+ Buildings (Add/Goal) --Goal Is To Retexture Alot Of Buildings

## (Requirement Specs)
![Specs](https://cdn.discordapp.com/attachments/688435303240957992/1017946622048620634/x6awf3A_d.png)


## PLEASE MAKE SURE YOU ARE DO ALL THE STEPS! (Requirement)

Okay So Go To Your FiveM Files ```/FiveM.app\citizen``` And Replace Every File In ```Citizen```

Okay So Go To Your OpenIV And Drag Everything In ```(Drag This Files In The Gta5 Mods Folder)```

- Using OpenlV install FSGraphics.oiv package (Not Done Yet)
- Run And Install Reshade


## SHOWCASE
_{ReshadeMe preset Is On My Discord}_
![1](https://cdn.discordapp.com/attachments/1021700112776437760/1090081781769838612/image.png) 

![2](https://cdn.discordapp.com/attachments/1021700112776437760/1090080555208552468/image_1.png)

![3](https://cdn.discordapp.com/attachments/1021700112776437760/1050153805666340894/image_2.png)

![4](https://cdn.discordapp.com/attachments/1021700112776437760/1090079640627990568/image.png)


## Installation For SoundPack 
Okay So Go To Your GTAV Files ```GrandTheftAutoV\x64\audio\sfx``` And Replace Every File In ```sfx```

## Here Is The Download For The SoundPack Cuz Is Too Big For Github

- [download](https://www.mediafire.com/file/vlhgr919ptod4s2/SoundPack.rar/file)

![here](https://cdn.discordapp.com/attachments/688435303240957992/1012911631648428112/unknown.png)

## ServerSide Graphics
 Simple Graphics quick Drag And Drop And Run
![server](https://github.com/IDKFORCE/fs-serversidegraphics/)

# Known Bug That Happen ClientSide
 Random Bloom (Client)
 Random Road Texture Loss (Client)
 Random Water Texture Loss (Client)
 
## Useful Information
- Okay Now IF The Graphics Break For Any Reason Make Sure To Delete The File ```content_index``` Now This Will Reset Your ```Citizen``` Files Back To Normal
- Okay Now IF The Audio Break For Any Reason Make Sure To Drag Your Files In The ```ArchiveFix.exe```
- Make Sure You Dont Have Any Other Graphics Pack As I Havent Try Messing With Other Graphics Pack
- Uninstall: Delete mods folder, ```dxgi.dll``` and ```d3d11.dll```

## Neccesary files links:

- [OpenlV](https://discord.com/channels/869166393470357535/1052957487160103042)
- [Winrar](https://www.win-rar.com/start.html?&L=0)

## Disclaimer
- Please reupload <3


## Remember If You :star2:Star This Repository It Will Keep You Notify For Future Updates

- :black_heart:Thank You For Downloading FS-GRAPHICS:black_heart:

# FS-Gráficos
FiveM Graphics Made By (⸸IDK_FORCE♥♱⸸♱♥#7611) (FiveM Graphics)
- :gear:・Únete a mi Discord y abre el ticket para pedir soporte o darme sugerencias o informar de un error
'''Mi discordia'''
- [Discordia](https://discord.gg/6kJ5ubDEWE)

```
  🎅🎃🍁Actualizaciones🍁🎃🎅 de septiembre/octubre/noviembre/diciembre/enero
```
  - Mejor agua
  - Mejor lluvia
  - Mejores colores
  - Mejores reflexiones
  - Mejores nubes
  - Mejores Noches
  - Mejor cielo
  - Mejor fuego
  - Mejores farolas
  - Aumento de FPS

```
   Cosas para (arreglar/agregar)
```
- Random Bloom en vehículos policiales (arreglo)
- Gotas de lluvia en vehículos (arreglo)
- Agua sucia (arreglar)
- Mejor niebla (añadir)
- Charcos de lluvia (añadir)
- Mejores carreteras/tierra/hierba (añadir)
- Mejor vegetación (añadir)
- Mejor mezcla de texturas (agregar)
- Mejores sombras (añadir)
- Retexturizar más de 35+ edificios (Agregar / Objetivo) --El objetivo es retexturizar muchos edificios

## (Especificaciones de requisitos)
! [Especificaciones](https://cdn.discordapp.com/attachments/688435303240957992/1017946622048620634/x6awf3A_d.png)

## ¡POR FAVOR, ASEGÚRESE DE HACER TODOS LOS PASOS! (Requisito)

Bien, así que vaya a sus archivos FiveM '''/FiveM.appcitizen''' y reemplace todos los archivos en '''citizen'''

Bien, así que ve a tu OpenIV y arrastra todo en ''' (arrastra estos archivos en la carpeta Gta5 Mods)'''

- Uso de OpenlV instalar el paquete FSGraphics.oiv (aún no hecho)
- Ejecutar e instalar Reshade

## ESCAPARATE
_{ReshadeMe preset Is On My Discord}_

![1](https://cdn.discordapp.com/attachments/1021700112776437760/1090081781769838612/image.png) 

![2](https://cdn.discordapp.com/attachments/1021700112776437760/1090080555208552468/image_1.png)

![3](https://cdn.discordapp.com/attachments/1021700112776437760/1050153805666340894/image_2.png)

![4](https://cdn.discordapp.com/attachments/1021700112776437760/1090079640627990568/image.png)

## Instalación para SoundPack 
Está bien, vaya a sus archivos GTAV '''GrandTheftAutoV  x64  audio  sfx ''' y reemplace todos los archivos en ''sfx'''

## Aquí está la descarga para el SoundPack Cuz es demasiado grande para Github

- [descargar](https://www.mediafire.com/file/vlhgr919ptod4s2/SoundPack.rar/file)

! [aquí](https://cdn.discordapp.com/attachments/688435303240957992/1012911631648428112/unknown.png)

## Gráficos ServerSide
 Gráficos simples rápido arrastrar y soltar y ejecutar
![server](https://github.com/IDKFORCE/fs-serversidegraphics/)

# Error conocido que sucede ClientSide
 Random Bloom (Cliente)
 Pérdida aleatoria de textura de carretera (cliente)
 Pérdida aleatoria de textura de agua (cliente)

## Información útil
- De acuerdo, ahora SI los gráficos se rompen por cualquier motivo, asegúrese de eliminar el archivo ''content_index' ahora esto restablecerá sus archivos '''Citizen''' a la normalidad
- Bien, ahora SI el audio se interrumpe por cualquier motivo, asegúrese de arrastrar sus archivos en el ''ArchiveFix.exe'''
- Asegúrese de no tener ningún otro paquete de gráficos, ya que no he intentado jugar con otro paquete de gráficos
- Desinstalar: Eliminar la carpeta de mods, ''dxgi.dll''' y ''d3d11.dll'''

## Enlaces a archivos necesarios:

- [OpenlV](https://discord.com/channels/869166393470357535/1052957487160103042)
- [Winrar](https://www.win-rar.com/start.html?&L=0)

## Descargo de responsabilidad
- Por favor, vuelva a subir <3

## Recuerda que si :star2:Star este repositorio te mantendrá notificado para futuras actualizaciones

- :black_heart:Gracias por descargar FS-GRAPHICS:black_heart:
